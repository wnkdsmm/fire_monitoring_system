...
def _normalize_context_label(value: Any) -> str:
    ...
    if text == "Р РЋР ВµР В»РЎРЉРЎРѓР С”Р В°РЎРЏ РЎвЂљР ВµРЎР‚РЎР‚Р С‘РЎвЂљР С•РЎР‚Р С‘РЎРЏ":
        return "РЎРµР»СЊСЃРєР°СЏ С‚РµСЂСЂРёС‚РѕСЂРёСЏ"
    if text == "Р СћР ВµРЎР‚РЎР‚Р С‘РЎвЂљР С•РЎР‚Р С‘РЎРЏ Р В±Р ВµР В· Р Р†РЎвЂ№РЎР‚Р В°Р В¶Р ВµР Р…Р Р…Р С•Р С–Р С• РЎРѓР ВµР В»РЎРЉРЎРѓР С”Р С•Р С–Р С• Р С—РЎР‚Р С•РЎвЂћР С‘Р В»РЎРЏ":
        return "РўРµСЂСЂРёС‚РѕСЂРёСЏ Р±РµР· РІС‹СЂР°Р¶РµРЅРЅРѕРіРѕ СЃРµР»СЊСЃРєРѕРіРѕ РїСЂРѕС„РёР»СЏ"
    return text
...
def _build_management_snapshot(
    ...
    precomputed_summary: SummaryResult | None = None,
    precomputed_risk_payload: dict[str, Any] | None = None,
) -> dict[str, Any]:  # one-off
    if not selected_tables:
        return _empty_management_snapshot(priority_horizon_days=planning_horizon_days)
    _ = precomputed_summary or summary
    tone_map = {"high": "fire", "medium": "sand", "low": "sky"}
    if precomputed_risk_payload is not None:
        risk_payload = precomputed_risk_payload
    else:
        try:
        ...
        territories.append(
            {
                ...
                "context_label": _normalize_context_label(item.get("settlement_context_label")) or "РљРѕРЅС‚РµРєСЃС‚ РЅРµ СѓРєР°Р·Р°РЅ",
