            ...
            actions.append(
                {
                    ...
                    "detail": f"В текущем срезе чаще всего фиксируется причина В«{dominant_cause['label']}В» ({dominant_cause['value_display']}).",
                }
            )
        if top_district:
            actions.append(
            ...
            actions.append(
                {
                    ...
                    "detail": f"{trend.get('description') or 'Динамика требует уточнения.'} РР·РјРµРЅРµРЅРёРµ: {trend.get('delta_display') or '0'}.",
                }
            )
