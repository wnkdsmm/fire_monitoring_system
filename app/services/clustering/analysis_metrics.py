...
def _build_representative_rows(
    ...
    for cluster_id, cluster_label in enumerate(cluster_labels):
        ...
        for row_index in nearest_indexes:
            ...
            row_values = [
                cluster_label,
                str(entity_row.get("Территория", "вЂ”")),
                str(entity_row.get("Район", "вЂ”")),
                str(entity_row.get("Тип территории", "вЂ”")),
            ]
...
def _build_notes(
    ...
    if cluster_profiles:
        ...
        notes.append(
            f"Самый крупный тип территорий сейчас вЂ” {largest['cluster_label']} ({largest['share_display']} выборки): {largest['segment_title'].lower()}."
        )
    notes.append(
