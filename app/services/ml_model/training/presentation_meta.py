...
def _build_notes(
    ...
    if not ml_result.get('is_ready') and filtered_records_count > 0:
        append_note('РСЃС‚РѕСЂРёРё пока недостаточно, чтобы показать устойчивый ML-прогноз и проверку качества.')
    if len(daily_history) < 60:
        append_note('РСЃС‚РѕСЂРёРё меньше 60 дней: для корректной ML-валидации этого обычно недостаточно.')
