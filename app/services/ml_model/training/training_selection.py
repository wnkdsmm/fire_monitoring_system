...
def _build_count_selection_details(
    ...
    if baseline_delta is not None:
        long_reason += f' Изменение MAE относительно seasonal baseline СЃРѕСЃС‚Р°РІРёР»Рѕ {baseline_delta * 100.0:+.1f}%.'
    ...
    if heuristic_improvement is not None and selected_count_model_key != 'heuristic_forecast':
        long_reason += f' Изменение MAE относительно heuristic forecast СЃРѕСЃС‚Р°РІРёР»Рѕ {heuristic_improvement * 100.0:+.1f}%.'
