...
def _build_count_selection_details(
    ...
    if baseline_delta is not None:
        long_reason += f' РР·РјРµРЅРµРЅРёРµ MAE относительно seasonal baseline составило {baseline_delta * 100.0:+.1f}%.'
    ...
    if heuristic_improvement is not None and selected_count_model_key != 'heuristic_forecast':
        long_reason += f' РР·РјРµРЅРµРЅРёРµ MAE относительно heuristic forecast составило {heuristic_improvement * 100.0:+.1f}%.'
    ...
    for model_key in COUNT_MODEL_KEYS:
        ...
        rows.append(
            CountComparisonRow(
                ...
                role_label='РРЅС‚РµСЂРїСЂРµС‚РёСЂСѓРµРјР°СЏ count-модель',
