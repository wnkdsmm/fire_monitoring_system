...
def test_compare_series_for_years_outside_facts_uses_ml_and_keeps_rows_non_empty() -> None:
    payload = {"filters": {}, "compare_series": {}}
    daily_history = [
        {"date": date(2024, 5, 1), "count": 3.0},
        {"date": date(2025, 5, 2), "count": 5.0},
        {"date": date(2026, 5, 3), "count": 7.0},
    ]
    ...
    def _fake_train(*_args, **kwargs):
        ...
        return {
            "forecast_rows": [
                {"date": f"{target_year:04d}-{target_month:02d}-01", "forecast_value": 1.5},
                {"date": f"{target_year:04d}-{target_month:02d}-02", "forecast_value": 2.5},
                {"date": f"{target_year:04d}-{target_month:02d}-03", "forecast_value": 3.5},
            ]
        }
