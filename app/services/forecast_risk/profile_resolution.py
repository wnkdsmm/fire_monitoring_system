    ...
    if not windows_bundle.get("is_ready") or len(windows_bundle.get("windows") or []) < MIN_CALIBRATION_WINDOWS:
        return _build_expert_fallback_profile(
            ...
            reason=(
                ...
                or "РСЃС‚РѕСЂРёРё пока недостаточно для устойчивой калибровки весов по историческим окнам."
            ),
        )
...
def _build_expert_fallback_profile(
    ...
    profile["description"] = (
        "РСЃС‚РѕСЂРёРё пока недостаточно для устойчивой калибровки весов, поэтому сервис использует экспертный профиль как резервный режим."
    )
    summary = (
    ...
    calibration.update(
        {
            ...
            "notes": _unique_non_empty(
                ...
                + [reason, "РСЃРїРѕР»СЊР·РѕРІР°РЅС‹ экспертные веса как fallback."]
            ),
        }
    )
    profile["notes"] = notes
...
def _build_expert_retained_profile(
    ...
    summary = (
        "РСЃС‚РѕСЂРёС‡РµСЃРєРёРµ окна проверены, но альтернативные веса не показали устойчивого выигрыша по ranking-метрикам. "
