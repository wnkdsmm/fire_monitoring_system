...
def _territory_row_payload(
    ...
    return {
        ...
        "explanation": (
            f"РС‚РѕРіРѕРІС‹Р№ риск { _format_number(risk_score) } / 100. "
...
def _component_rationale(component_key: str, score: float, context: ScoreContext) -> str:
    ...
    if component_key == "consequence_severity":
        ...
        if not parts:
            parts.append("РСЃС‚РѕСЂРёСЏ тяжёлых последствий пока умеренная.")
        return " ".join(parts[:4])
    ...
    if context["tanker_dependency"] >= 0.55:
        parts.append("РР·-за удалённости территория сильнее зависит от подвоза воды.")
    if context["is_rural"]:
