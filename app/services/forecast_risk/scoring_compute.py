...
def _territory_row_payload(
    ...
    return {
        ...
        "explanation": (
            f"Р ВРЎвЂљР С•Р С–Р С•Р Р†РЎвЂ№Р в„– СЂРёСЃРє { _format_number(risk_score) } / 100. "
...
def _component_rationale(component_key: str, score: float, context: ScoreContext) -> str:
    ...
    if component_key == "consequence_severity":
        ...
        if not parts:
            parts.append("Р ВРЎРѓРЎвЂљР С•РЎР‚Р С‘РЎРЏ С‚СЏР¶С‘Р»С‹С… РїРѕСЃР»РµРґСЃС‚РІРёР№ РїРѕРєР° СѓРјРµСЂРµРЅРЅР°СЏ.")
        return " ".join(parts[:4])
    ...
    if context["tanker_dependency"] >= 0.55:
        parts.append("Р ВР В·-Р·Р° СѓРґР°Р»С‘РЅРЅРѕСЃС‚Рё С‚РµСЂСЂРёС‚РѕСЂРёСЏ СЃРёР»СЊРЅРµРµ Р·Р°РІРёСЃРёС‚ РѕС‚ РїРѕРґРІРѕР·Р° РІРѕРґС‹.")
    if context["is_rural"]:
