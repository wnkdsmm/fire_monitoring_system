...
def import_uploaded_data(
    ...
    add_log(session_id, resolved_job_id, f"РРјСЏ проекта: {settings.project_name}")
    add_log(session_id, resolved_job_id, f"Папка результатов: {settings.output_folder}")
    ...
    try:
        ...
        _invalidate_runtime_caches(session_id, resolved_job_id)
        add_log(session_id, resolved_job_id, f"РРјРїРѕСЂС‚ завершён: {uploaded_file_path}")
        final_status = "completed"

        if step.data is not None:
            return {
                "status": "РРјРїРѕСЂС‚ выполнен успешно",
        ...
        return {
            "status": "РРјРїРѕСЂС‚ завершён, но данные недоступны",
            "rows": 0,
            "columns": 0,
            "project_name": settings.project_name,
            "job_id": resolved_job_id,
