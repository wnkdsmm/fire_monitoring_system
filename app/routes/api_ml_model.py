...
def get_ml_causes_chart_data(**kwargs):
    from app.services.ml_model.core import get_ml_causes_chart_data as _get_ml_causes_chart_data

    return _get_ml_causes_chart_data(**kwargs)
...
@router.post("/api/ml-causes-chart")
def ml_causes_chart_endpoint(request: Request, payload: dict = Body(...)):
    try:
        raw_table_names = payload.get("table_names")
        table_names = (
            [str(item or "").strip() for item in raw_table_names if str(item or "").strip()]
        ...
        return run_session_json_action(
            ...
            lambda _session_id: {
                "status": "completed",
                "result": get_ml_causes_chart_data(
                    table_name=str(payload.get("table_name") or "all"),
                    table_names=table_names,
                    cause=str(payload.get("cause") or "all"),
                    object_category=str(payload.get("object_category") or "all"),
